 ...
 * @buffer: full buffer written in promp
 * @cmd: command written after promp
 * @options: options of the comman
 * @path: path of the cmd if foun, else cmd itself
 * @errnum: error count
 ...
 * @unsetnull: check for settig environment to NULL
 *
...
char **checkInput(int ac, char **av, size_t *bufsize, char **buffer, hshpack *shpack);
hshpack *set_struct(char *argv0, int *errn, int *exnum, int *relation, int *run_able, char ***env, int *unsetnull);
